...
## load dataset
dataset = read.csv("data/dataset_lake_stream_v0_1.csv")
head(dataset)
...
s    = which(is.na(dataset$CdBH))
dataset = dataset[-s,]

## response variable
# Y      = std(dataset$max_troph_lvl)
Y      = std(dataset$connectance)
n_data = length(Y)

## means and stds
temp_mean = mean(dataset$temp,na.rm=T)
temp_sd   = sd(dataset$temp,na.rm=T)
bod_mean  = mean(dataset$dbo,na.rm=T)
bod_sd    = sd(dataset$dbo,na.rm=T)
Y_mean    = mean(dataset$connectance,na.rm=T)
Y_sd      = sd(dataset$connectance,na.rm=T)

## explanatory variables
type    = (dataset$type == "lake")*1
temp    = std(dataset$temp)
bod     = std(dataset$dbo)
rich    = std(dataset$richness)
year    = std(dataset$year)
alt     = std(dataset$altitude)
long    = std(dataset$long)
latt    = std(dataset$lat)
locat   = stringToInteger(dataset$CdBH)

## matrix of explanatory variables
X_obs       =                 cbind(1, type,temp,temp^2,temp*type,bod,bod^2,type*bod,temp*bod,temp*bod*type,rich,year)
X_mis_l     =                 cbind(1, type,temp,temp^2,temp*type,  1,    1,type*  1,temp*  1,temp*  1*type,rich,year)
X_mis_r     = function(x)     cbind(1,    1,   1,     1,        1,  x,  x^2,      x,       x,            x,   1,  1)
X_pred      = function(x,y,i) cbind(1,(i-1),   x,   x^2,  x*(i-1),  y,  y^2,y*(i-1),     x*y,    x*y*(i-1),   0,  0)
# ddx.X_pred  = function(x,y,i) cbind(0,    0,   1,   2*x,    (i-1),  0,    0,      0,       y,      y*(i-1),   0,  0)
# ddy.X_pred  = function(x,y,i) cbind(0,    0,   0,     0,        0,  1,  2*y,  (i-1),       x,      x*(i-1),   0,  0)
# ddX.X_pred  = function(x,y,i) cbind(0,    0,   1,   2*x,    (i-1),  0,    0,      0,       y,      y*(i-1),   0,  0) + 
#                               cbind(0,    0,   0,     0,        0,  1,  2*y,  (i-1),       x,      x*(i-1),   0,  0)
n_beta  = ncol(X_obs)
colnames(X_obs) = colnames(X_obs) = c("Intercept","Ecosystem Type","Temperature","Temperature^2","Temperature * Ecosystem Type","BOD","BOD^2","Ecosystem Type * BOD","Temperature * BOD","Temperature * BOD * Ecosystem Type","Species Richness","Year")

## missing values
idx_mis = which(is.na(bod))
n_mis   = length(idx_mis)

## splitting dataset into observed and missing
Y_obs   = Y[-idx_mis]
X_obs   = X_obs[-idx_mis,]
Y_mis   = Y[idx_mis]
