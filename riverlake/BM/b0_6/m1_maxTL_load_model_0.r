...
X_obs   = cbind(1,type,temp,temp^2,type*temp,dbo,dbo^2,type*dbo,year,alt)
X_mis   = cbind(1,type,temp,temp^2,type*temp,  1,    1,type    ,year,alt)
n_beta  = ncol(X_obs)
idx_par_mis = c(6,7,8)
...
{
	Sigma = sd_lik*t(sd_lik)*ac(DM,rho) # sd_i * sd_i on diagonal and sd_i * sd_j * rho_ij off diagonal
	return(Sigma)
}
...
logLik = function(Y_obs,X_obs,Y_mis,X_mis,x_mis,beta,sd_lik,rho,DM)
{
	...
	X_mis[,idx_par_mis] = X_mis[,idx_par_mis] * cbind(x_mis,x_mis^2,x_mis)
