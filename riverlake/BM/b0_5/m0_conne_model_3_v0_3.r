...
X_obs   = cbind(1,year,temp,temp^2,type,dbo,type*dbo,alt,rich)
X_mis   = cbind(1,year,temp,temp^2,type,  1,type*1  ,alt,rich)
n_beta  = ncol(X_obs)
...
{
    X_mis[,6:7] = X_mis[,6:7] * cbind(x_mis,x_mis)
...
for(i in 1:2)
{
    ...
    X_pred   = cbind(1,0,x,x^2,(i-1),0,0,0,0)
...
for(i in 1:2)
{
    ...
    X_pred   = cbind(1,0,0,0,(i-1),x,(i-1)*x,0,0)
    pred     = chainList.apply(chainList_thinned,function(x_) Yhat(X_pred,x_[-1][n_sd_lik + 1 + 1 + 1:n_beta]))
    polygon(x=c(X_pred[,6],rev(X_pred[,6])),y=c(pred$f_q0.05,rev(pred$f_q0.95)),border=NA,col=adjustcolor(i+1,alpha=0.5))
    # lines(X_pred[,7],Yhat(X_pred,beta_map),col=i+1)
    lines(X_pred[,6],pred$f_mean,col=i+1)
}
#
legend("topright",legend=c("Stream","Lake"),lty=1,col=1:2+1,bty="n")
#
...
X_mis_      = X_mis
X_mis_[,6:7] = X_mis_[,6:7] * cbind(x_mis_,x_mis_)
...
colnames(chain_) = c("P","1","year","temp","temp^2","type","dbo","type*dbo","alt","rich")
