...
X_obs   = cbind(1,temp,temp^2,type,dbo,type*dbo,alt,rich)
X_mis   = cbind(1,temp,temp^2,type,  1,type*1  ,alt,rich)
n_beta  = ncol(X_obs)
...
{
    X_mis[,5:6] = X_mis[,5:6] * cbind(x_mis,x_mis)
...
#
plot(X_obs[,2],Y_obs,pch=16,col=adjustcolor("black",alpha=0.5),xlab="Temperature (SU)",ylab=response,main=paste(response," ~ temperature",sep=""))
for(i in 1:2)
{
    ## predictions
    x        = seq(min(temp),max(temp),0.1)
    X_pred   = cbind(1,x,x^2,(i-1),0,0,0,0)
    ...
    lines(X_pred[,2],pred$f_mean,col=i+1)
}
#
...
#
plot(X_obs[,5],Y_obs,pch=16,col=adjustcolor("black",alpha=0.5),xlab="DBO (SU)",ylab=response,main=paste(response," ~ DBO",sep=""))
for(i in 1:2)
{
    ## predictions
    x        = seq(min(dbo,na.rm=T),max(dbo,na.rm=T),0.1)
    X_pred   = cbind(1,0,0,(i-1),x,(i-1)*x,0,0)
    pred     = chainList.apply(chainList_thinned,function(x_) Yhat(X_pred,x_[-1][n_sd_lik + 1 + 1 + 1:n_beta]))
    polygon(x=c(X_pred[,5],rev(X_pred[,5])),y=c(pred$f_q0.05,rev(pred$f_q0.95)),border=NA,col=adjustcolor(i+1,alpha=0.5))
    # lines(X_pred[,7],Yhat(X_pred,beta_map),col=i+1)
    lines(X_pred[,5],pred$f_mean,col=i+1)
}
#
legend("topright",legend=c("Stream","Lake"),lty=1,col=1:2+1,bty="n")
#
...
X_mis_      = X_mis
X_mis_[,5:6] = X_mis_[,5:6] * cbind(x_mis_,x_mis_)
...
colnames(chain_) = c("P","1","temp","temp^2","type","dbo","type*dbo","alt","rich")
